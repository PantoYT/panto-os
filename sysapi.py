...
HONEYPOT_LOG = os.path.join(os.path.dirname(__file__), '..', 'honeypot', 'honeypot.log')
HONEYPOT_KNOWN_ROUTES = {
    '/stats', '/history', '/docker', '/tailscale', '/bots',
    '/run-script', '/system/shutdown', '/system/restart',
    '/system/cancel', '/system/lock', '/docker/restart-all',
    '/hp-log',
}

def hp_log(event, data):
    try:
...
VRED_CREDS = os.path.join(os.path.dirname(__file__), '..', 'DiscordBots', 'Vred', 'credentials.json')

def _get_vulcan_schedule(target_date):
    """Fetch lessons for target_date (datetime) using Vred's VulcanClient."""
    import sys
    vred_path = os.path.join(os.path.dirname(__file__), '..', 'DiscordBots', 'Vred')
    if vred_path not in sys.path:
        sys.path.insert(0, vred_path)
    from client import VulcanClient
    client = VulcanClient(credentials_path=VRED_CREDS)
    lessons = client.get_lessons(target_date, target_date)
    result = []
    for l in sorted(lessons, key=lambda x: x.get('TimeSlot', {}).get('Position', 99)):
        slot    = l.get('TimeSlot', {})
        display = slot.get('Display', '')   # e.g. "08:00-08:45"
        pos     = slot.get('Position', 0)
        start_h = None
        if display and '-' in display:
            try:
                start_h = int(display.split('-')[0].split(':')[0])
            except Exception:
                pass
        subject = l.get('Subject', {}).get('Name', '?')
        room    = l.get('Room')
        room_s  = room['Code'] if room else None
        change  = bool(l.get('Change'))
        teacher = l.get('TeacherPrimary', {})
        teacher_s = teacher.get('DisplayName', '') if teacher else ''
        result.append({
            'position': pos,
            'hour':     start_h,
            'display':  display,
...
def schedule_today():
    return _schedule_for_date(datetime.datetime.now())
...
def schedule_date(date_str):
    ...
    return _schedule_for_date(d)

def _schedule_for_date(d):
    try:
        lessons = _get_vulcan_schedule(d)
        return jsonify({'lessons': lessons, 'date': d.strftime('%Y-%m-%d'), 'ok': True})
    except Exception as e:
        return jsonify({'lessons': [], 'ok': False, 'error': str(e)})

@app.route('/schedule-raw/<date_str>')
def schedule_raw(date_str):
    """Debug: zwraca surowe dane z Vulcan API (pola TimeSlot, Position, Display)."""
    try:
        import sys
        vred_path = os.path.join(os.path.dirname(__file__), '..', 'DiscordBots', 'Vred')
        if vred_path not in sys.path:
        ...
        client = VulcanClient(credentials_path=VRED_CREDS)
        raw = client.get_lessons(d, d)
        simplified = [
            {
                'pos':     l.get('TimeSlot', {}).get('Position'),
                'display': l.get('TimeSlot', {}).get('Display'),
                'subject': l.get('Subject', {}).get('Name'),
                'change':  bool(l.get('Change')),
            }
            for l in raw
        ]
        return jsonify({'count': len(raw), 'lessons': simplified, 'ok': True})
    except Exception as e:
        return jsonify({'ok': False, 'error': str(e)})

DISCORD_BOTS = [
    {'name': 'Fred',  'label': 'Epic Games Bot', 'script': 'Fred\\main.py'},
    {'name': 'Tred',  'label': 'Trivia Bot',      'script': 'Tred\\main.py'},
    {'name': 'Qred',  'label': 'Quotes Bot',      'script': 'Qred\\main.py'},
    {'name': 'Vred',  'label': 'EduVulcan Bot',   'script': 'DiscordBots\\Vred\\main.py'},
]
...
def bots_status():
    ...
    for bot in DISCORD_BOTS:
        online = False
        pid = None
        # bot script dir e.g. "Fred" or "DiscordBots\\Vred"
        bot_dir = bot['script'].replace('\\', '/').rsplit('/', 1)[0].lower()
        for p in psutil.process_iter(['pid', 'cmdline']):
            try:
                cmdline = ' '.join(p.info['cmdline'] or []).replace('\\', '/')
                # full path match
                if bot['script'].replace('\\', '/') in cmdline:
                    online = True; pid = p.info['pid']; break
                # cwd match for scripts launched as plain "main.py"
                ...
                pass
        results.append({'name': bot['name'], 'label': bot['label'], 'online': online, 'pid': pid})
...
@app.route('/hp-log', methods=['POST', 'OPTIONS'])
def honeypot_log_endpoint():
    if request.method == 'OPTIONS':
        return '', 204
    try:
        data = request.get_json(force=True, silent=True) or {}
        hp_log('hp_frontend', data)
        if data.get('event') == 'login_attempt':
            hp_log('CREDENTIAL_CAPTURE', {
            ...
            if DISCORD_WEBHOOK:
                try:
                    requests.post(DISCORD_WEBHOOK, json={
                        'content': f'🍯 **Honeypot hit!**\nIP: `{request.headers.get("X-Forwarded-For", request.remote_addr)}`\nUser: `{data.get("username","?")}` Pass: `{data.get("password","?")}`\nUA: `{request.headers.get("User-Agent","")[:80]}`'
                    }, timeout=5)
                except Exception:
                    pass
    except Exception:
...
def honeypot_404(e):
    ...
    # Return a convincing fake response for common scanner targets
    fake_paths = {
        '/wp-login.php': ('WordPress Login', 200),
        '/admin': ('Admin Panel', 200),
        '/login': ('Login', 200),
        '/phpmyadmin': ('phpMyAdmin', 200),
        '/.env': ('', 200),
        '/config': ('{}', 200),
    }
    for fp, (body, code) in fake_paths.items():
        if path.startswith(fp):
            hp_log('FAKE_RESPONSE', {'path': path, 'fake': fp})
            return body, code
